...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    first_iter += 1
    for iteration in range(first_iter, opt.iterations + 1):     #开始训练

        iter_start.record()  #开始记录event

        gaussians.update_learning_rate(iteration)  #高斯学习率更新
        ...
        mouth_window = (mouth_global_ub - mouth_global_lb) * 0.2   #在获取唇部窗口时去掉边缘区域
